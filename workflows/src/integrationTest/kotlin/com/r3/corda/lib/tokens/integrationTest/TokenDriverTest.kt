...
class TokenDriverTest {
    ...
    fun `should allow issuance of inline defined token`() {
        driver(
            ...
            issuer.rpc.startFlowDynamic(
                ...
                listOf(tokenToIssueToIssuer, tokenToIssueToOther),
                emptyList<Party>()
    ...
    fun `should allow retrieval of tokens by owning key`() {
        ...
        driver(
            ...
            issuer.rpc.startFlowDynamic(
                IssueTokens::class.java,
                listOf(tokenToIssueToIssuer, tokenToIssueToIssuerCi1, tokenToIssueToIssuerCi2),
                emptyList<Party>()
            ).returnValue.getOrThrow()
    ...
    fun `beefy tokens integration test`() {
        driver(
            ...
            assertThat(nodeB.rpc.vaultQueryBy<FungibleToken>(tokenAmountCriteria(GBP)).states.sumTokenStateAndRefsOrZero(GBP issuedBy issuerParty)).isEqualTo(
                Amount.zero(GBP issuedBy issuerParty)
            )
            ...
            val houseUpdateTx = issuer.rpc.startFlowDynamic(
                UpdateEvolvableToken::class.java,
                oldHouse,
                newHouse,
                emptyList<Party>()
            ).returnValue.getOrThrow()
            ...
            assertThat(issuer.rpc.vaultQueryBy<FungibleToken>(tokenAmountCriteria(GBP)).states.sumTokenStateAndRefsOrZero(GBP issuedBy issuerParty)).isEqualTo(
                Amount.zero(GBP issuedBy issuerParty)
            )
        }
    }
    ...
    fun `tokens locked in memory are still locked after restart`() {
        driver(
            ...
            // Restart the node
            val restartedNode =
                startNode(providedName = DUMMY_BANK_A_NAME, customOverrides = mapOf("p2pAddress" to "localhost:30000")).getOrThrow()
